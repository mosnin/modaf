...
/**
 * Block: Liquid glass primitives — LiquidButton + LiquidGlassCard (SVG-displacement
 * "liquid glass" refraction over shadcn Button/Card), plus a music-player demo card.
 * Source: KokonutUI (kokonutui.com, @dorianbaffier, MIT), v2.0.0.
 *
 * Import fixes applied: shadcn's current button doesn't export ButtonProps — derived
 * locally from the component instead.
 *
 * Adaptation notes:
 * - DIRECTION-GATED: this is full glassmorphism — one step from banned default #3.
 *   Allowed only where the chosen direction genuinely supports glass (e.g. Soft
 *   Luxury hero moment), on ONE surface, never as a card-grid treatment.
 * - `backdropFilter: url(#id)` SVG filters only render in Chromium; Safari/Firefox
 *   fall back to the inset-shadow glass rim, which is acceptable degradation.
 * - The demo's VolumeBars need this CSS (add to globals if used):
 *   @keyframes bounce-music { 0%,100% { height: 6px } 50% { height: 24px } }
 *   .animate-bounce-music { animation: bounce-music 1s ease-in-out infinite; }
 * - PLACEHOLDER content: album art URL, track metadata, hardcoded #FF2E55 pinks —
 *   retheme to the direction's accent. The NotificationCenter demo is reference
 *   only; the exports to reuse are LiquidButton and LiquidGlassCard.
 */
