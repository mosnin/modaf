...
| Dock | `npx motion-primitives@latest add dock` | Apple-style dock with smooth cursor-tracked magnification (motion values, not per-item state) — the **preferred implementation** of the app shell's collapsed dock (`internal/01_app_shell.md` § Collapsed Dock), upgrading the simpler `navigation/floating-dock-nav.tsx` block. Derive items from the route plan, match the shell's magnification spec, disable magnification under `prefers-reduced-motion`. |
| Glow effect | `npx motion-primitives@latest add glow-effect` | Animated glow behind an element — **tightest restraint of any vetted effect**: sits one step from the banned gradient-blob look, so it's allowed only as the site's single hero moment or on the one primary CTA, in the direction's accent, never on cards/grids, and never alongside border-trail on the same page. If the chosen direction is flat (Swiss, Editorial, Terminal/Brutalist), skip it entirely. |
| Image comparison | `npx motion-primitives@latest add image-comparison` | Draggable before/after image slider — transformation proof sections (cleanup tools, image/design products, redesign case studies). Both sides must be REAL product output, never mockups; a strong candidate for the page's interactive proof moment (`site_composition.md`). |
