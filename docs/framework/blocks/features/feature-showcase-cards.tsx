// Block: Feature showcase cards — corner-tick decorated cards with dual-mode product images
// and a full-width circular-motif statement card. Source: 21st.dev community (features-10).
// Adapt: real screenshots (dark + light), real copy, restyle per docs/project/09_design_direction.md.
                ...
                <div className="mx-auto grid gap-4 lg:grid-cols-2">
                    <FeatureCard>
                        <CardHeader className="pb-3">
                            <CardHeading
                                icon={MapIcon}
                                title="Replace feature label"
                                description="Replace with the feature's one-line mechanism."
                            />
                        </CardHeader>
                    ...
                    <FeatureCard className="p-6 lg:col-span-2">
                        <p className="mx-auto my-6 max-w-md text-balance text-center text-2xl font-semibold">Replace with the cross-feature statement that ties the grid together.</p>
                        ...
                        <div className="flex justify-center gap-6 overflow-hidden">
                            ...
                            <CircularUI label="Exclusion" circles={[{ pattern: 'primary' }, { pattern: 'none' }]} className="hidden sm:block" />
                        </div>
                    </FeatureCard>
                </div>
            </div>
...
const DualModeImage = ({ darkSrc, lightSrc, alt, width, height, className }: DualModeImageProps) => (
    <>
        {/* PLACEHOLDER images — replace with real product screenshots */}
        <img src={darkSrc} className={cn('hidden dark:block', className)} alt={`${alt} dark`} width={width} height={height} />
        <img src={lightSrc} className={cn('shadow dark:hidden', className)} alt={`${alt} light`} width={width} height={height} />
    </>
)
