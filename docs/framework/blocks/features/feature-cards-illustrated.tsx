// Block: Feature cards, illustrated — features paired with rich in-card product illustrations
// (AI chat, meeting card, code review). Source: 21st.dev community (two arrangements merged;
// the stacked 5-col variant is a trivial re-grid of the same pieces).
// Adapt: replace copy + placeholder avatars, restyle per docs/project/09_design_direction.md.
import { Button } from '@/components/ui/button'
import { Card } from '@/components/ui/card'
import { ArrowUp, Globe, Play, Plus, Signature, Sparkles } from 'lucide-react'

// PLACEHOLDER avatars — replace with real or product-appropriate imagery
...
export default function FeatureCardsIllustrated() {
    return (
        <section>
            <div className="py-24">
                <div className="mx-auto w-full max-w-3xl px-6">
                    ...
                    <div className="mt-12 grid gap-12 sm:grid-cols-2">
                        <div className="col-span-full space-y-4">
                            <Card className={`overflow-hidden px-6 sm:col-span-2 ${softCard}`}>
                            ...
                            <div className="max-w-md sm:col-span-3">
                                ...
                                <p className="text-muted-foreground mt-3 text-balance">Replace with how this product&apos;s assistant helps in one concrete sentence.</p>
                            </div>
                        </div>
                        <div className="grid grid-rows-[1fr_auto] space-y-4">
                            <Card className={`p-6 ${softCard}`}>
                                <MeetingIllustration />
                            </Card>
                            <div>
                                <h3 className="text-foreground text-lg font-semibold">Feature two</h3>
                                <p className="text-muted-foreground mt-3 text-balance">Replace with the mechanism this feature delivers.</p>
                            </div>
                        </div>

                        <div className="grid grid-rows-[1fr_auto] space-y-4">
                            <Card className={`overflow-hidden p-6 ${softCard}`}>
                                <CodeReviewIllustration />
                            </Card>
                            <div>
                                <h3 className="text-foreground text-lg font-semibold">Feature three</h3>
                                <p className="text-muted-foreground mt-3 text-balance">Replace with the mechanism this feature delivers.</p>
                            </div>
                        </div>
                    </div>
    ...
    return (
        <Card aria-hidden className="p-4 transition-transform duration-200 group-hover:translate-y-0">
            <div className="max-w-3/4 ml-auto w-fit">
                <p className="border-foreground/5 bg-foreground/5 mb-2 rounded-l-2xl rounded-t-2xl rounded-br border p-4 text-sm">Replace with a believable user message for THIS product.</p>
